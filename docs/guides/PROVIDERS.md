...
connection. A provider answers *"how does an event get in, and how is it
authenticated?"* — distinct from **routing**, which answers *"which agent/template
handles it?"* (see each agent's `routing:` block).
